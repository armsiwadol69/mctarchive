...
$taget_s = $_GET["year"];
$sql_search = "SELECT * FROM mctarchive WHERE sec LIKE '$taget_s' ORDER BY add_date ASC LIMIT $offset, $no_of_records_per_page";
$sql_search_2all = "SELECT * FROM mctarchive WHERE sec LIKE '$taget_s' ORDER BY id ASC ";
...
elseif (isset($_GET["teacher"]) == 1 AND empty($_GET["teacher"]) == 0) {
  ...
  $sql_search = "SELECT * FROM mctarchive WHERE teacher LIKE '$taget_s' ORDER BY add_date ASC LIMIT $offset, $no_of_records_per_page";
  $sql_search_2all = "SELECT * FROM mctarchive WHERE teacher LIKE '$taget_s' ORDER BY id ASC ";
  $taget_to_find = 'หัวข้อที่เกี่ยวกับที่ปรึกษา : '.$taget_s;
  $taget_nextpage = "?teacher="."$taget_s";
}
elseif (isset($_GET["branch"]) == 1 AND empty($_GET["branch"]) == 0) {
  $taget_s = $_GET["branch"];
  $sql_search = "SELECT * FROM mctarchive WHERE branch LIKE '$taget_s' ORDER BY add_date ASC LIMIT $offset, $no_of_records_per_page";
  $sql_search_2all = "SELECT * FROM mctarchive WHERE branch LIKE '$taget_s' ORDER BY id ASC ";
  $taget_to_find = 'หัวข้อที่เกี่ยวกับสาขา : '.$taget_s;
  $taget_nextpage = "?branch="."$taget_s";
}
elseif ($_GET["type_doc"] == "1" AND empty($_GET["type_doc"]) == 0) {
  $taget_s = $_GET["type_doc"];
  $sql_search = "SELECT * FROM mctarchive WHERE type_doc LIKE '1' ORDER BY add_date ASC LIMIT $offset, $no_of_records_per_page";
  $sql_search_2all = "SELECT * FROM mctarchive WHERE type_doc LIKE '1' ORDER BY id ASC ";
...
elseif ($_GET["type_doc"] == "2" AND empty($_GET["type_doc"]) == 0) {
  $taget_s = $_GET["type_doc"];
  $sql_search = "SELECT * FROM mctarchive WHERE type_doc LIKE '2' ORDER BY add_date ASC LIMIT $offset, $no_of_records_per_page";
    $sql_search_2all = "SELECT * FROM mctarchive WHERE type_doc LIKE '2' ORDER BY id ASC ";
...
<html lang="en" dir="ltr">
  <head>
    ...
    <meta property="og:title" content="<?php echo "$taget_to_find"; ?> | ระบบสืบค้นปริญญานิพนธ์และงานวิจัย คณะเทคโนโลยีสื่อสารมวลชน มหาวิทยาลัยเทคโนโลยีราชมงคลธัญบุรี" />
    <meta property="og:description" content="ระบบสืบค้นปริญญานิพนธ์และงานวิจัย คณะเทคโนโลยีสื่อสารมวลชน มหาวิทยาลัยเทคโนโลยีราชมงคลธัญบุรีี"/>
 ...
 include'navbar.php';
  ...
  <h4 class="">จำนวนรายการที่ตรงตามเงื่อนไข <span class="badge bg-dark text-while no-text-outline"> <?php echo "$row_cnt"; ?></span> รายการ</h4>
  <?php if ($row_cnt == 0) {
      ...
      <?php
            ...
            <div class="col-12">
              ...
              <div class="btn-group my-2 shadow-sm" role="group" aria-label="First group">
               ...
               <a class="btn btn-dark <?php if($page_default >= $total_pages){ echo 'disabled'; } ?>" href="<?php echo "$taget_nextpage"; ?>&page=<?php echo $total_pages; ?>">หน้าสุดท้าย</a>
